...
import { X, Play, Pause, Check, AlertCircle, Loader2, ExternalLink, Mail } from 'lucide-react';
...
export function CampagnaSendModal({ isOpen, onClose, campagna, onCompleted }: CampagnaSendModalProps) {
  ...
  // Carica destinatari e messaggio
  useEffect(() => {
    if (!isOpen) return;
    ...
    const load = async () => {
      try {
        ...
        const allClients = await comunicazioniService.getTargetClienti({}, campagna.canale);
        ...
        if (campagna.template_id) {
          // Usa il corpo del template (senza sostituzione — la faremo per ogni client)
  ...
  const startSending = () => {
    setStatus('sending');
    try { comunicazioniService.updateCampagnaStato(campagna.id, 'in_corso'); } catch (e) { console.error(e); }
    sendNext();
  };
            ...
            results.map((result, i) => {
              ...
              return (
                <div
                  ...
                  style={{ borderBottom: i < results.length - 1 ? '1px solid var(--glass-border)' : undefined }}
                >
                  {/* Status icon */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {result.error && (
                      <p className="text-[10px] truncate" style={{ color: 'var(--color-danger)' }}>{result.error}</p>
                    )}
                  </div>
                </div>
              );
          ...
          {status === 'completed' ? (
            ...
            <>
              ...
              <button
                ...
                style={{ background: 'var(--color-primary)' }}
              >
                {status === 'ready' ? 'Avvia' : `Invia prossimo (${currentIndex + 1}/${total})`}
              </button>
            </>
          )}
        </div>
